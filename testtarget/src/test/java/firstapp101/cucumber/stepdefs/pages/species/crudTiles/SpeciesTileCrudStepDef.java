 ...
 */
package firstapp101.cucumber.stepdefs.pages.species.crudTiles;

import firstapp101.cucumber.pom.pages.species.crudTiles.edit.SpeciesCrudTileEdit;
import firstapp101.cucumber.pom.pages.species.crudTiles.list.SpeciesCrudTileList;
...
@Slf4j
@ScenarioScoped
public class SpeciesTileCrudStepDef extends AbstractStepDef {

	@Inject
	SpeciesCrudTileEdit speciesCrudTileEdit;

	@Inject
	SpeciesCrudTileList speciesCrudTileList;
	 ...
	 */
	private AbstractEntity speciesEntity;
	...
	@When("I create references for crud tile Species in page Species")
	public void Species() throws Exception {
		speciesEntity = getEntityFactory("Species").createWithNoRef();
		for (EntityReference reference : speciesEntity.References) {
	...
	public void clickCreateButton() throws Exception {
		speciesCrudTileList.createButton.click();
	}

	@Then("I create a valid entity in crud tile Species in page Species")
	public void createValidEntityInTile() throws Exception {
		speciesCrudTileEdit.applyEntity(speciesEntity);
	}

	@Then("I verify Species list in page Species")
	public void verifyCrudTileList() throws Exception {
		webDriverWait.until(x -> speciesCrudTileList.CrudListItems.size() > 0);
	}

	@When("I update an existing entity in crud tile Species of page Species")
	public void updateEntityInFrontend() throws Exception {
		...
		webDriverWait.until(x -> speciesCrudTileList.CrudListItems.size() > 0);
		speciesCrudTileList.EditButtons.get(0).click();
		speciesEntity = getEntityFactory("Species").createWithNoRef();
		speciesCrudTileEdit.applyEntity(speciesEntity);
	...
	@When("I view an existing entity in crud tile Species of page Species")
	public void viewEntityInFrontend() throws Exception {
		...
		// wait for the list to be populated with values
		webDriverWait.until(x -> speciesCrudTileList.CrudListItems.size() > 0);
		speciesCrudTileList.ViewButtons.get(0).click();
	...
	@When("I delete an existing entity in crud tile Species of page Species")
	public void deleteEntityInFrontend() throws Exception {
		...
		speciesCrudTileList.DeleteButtons.get(0).click();
